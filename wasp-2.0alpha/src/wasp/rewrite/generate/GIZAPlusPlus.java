...
/**
 * An E-to-F translation model based on IBM Model 4.  This is the only translation model that the Rewrite
...
public class GIZAPlusPlus extends TranslationModel {
	...
	private class Settings {
	    public String[][] config = {
	            ...
	            {"model4iterations", "3"},
	            {"model5iterations", "3"},
	            {"pegging", (USE_MRL_GRAMMAR) ? "1" : "0"},  // saves time
	            {"compactadtable", "0"}
	    };
            ...
            if (USE_MRL_GRAMMAR) {
	            ...
	            for (short j = 0; j < F.length; ++j)
	            	if (!F[j].isUnary() && !Config.getMRLGrammar().isZeroFertility(F[j])) {
		            	String token = RewriteModel.token(F[j]);
		            	tarSentOut.print(token);
		            	tarSentOut.print(' ');
		                sentOut.print(tarVocab.getId(token, true));
		                sentOut.print(' ');
		            }
            } else {
            	Symbol[] F = ex.F.syms;
        	...
        	if (rules[i].countArgs() == 0 && !rules[i].isWildcard()) {
	        	...
	        	for (short j = 0; j < E.length; ++j)
	        		if (!((Terminal) E[j]).isBoundary()) {
	            		...
	            		srcSentOut.print(token);
	            		srcSentOut.print(' ');
		                sentOut.print(srcVocab.getId(token, true));
		                sentOut.print(' ');
	        		}
	        	srcSentOut.println();
	        	sentOut.println();
	        	...
	        	if (USE_MRL_GRAMMAR) {
		        	...
		        	String token = RewriteModel.token(F);
		        	tarSentOut.print(token);
		        	sentOut.print(tarVocab.getId(token, true));
	        	} else {
            	...
            	if (F[j].isWildcardMatch())
            		for (short k = 0; k < E.length; ++k)
            			if (E[k].equals(F[j].getRhs((short) 0))) {
				            ...
				            sentOut.println(srcVocab.getId(token, true));
				            // target is F
				            token = RewriteModel.token(F[j]);
				            tarSentOut.println(token);
				            sentOut.println(tarVocab.getId(token, true));
				            break;
			            }
        }
