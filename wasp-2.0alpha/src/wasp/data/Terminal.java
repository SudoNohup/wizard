...
/**
 * The class for terminal symbols.  For logical variables, see the <code>Variable</code> class.
 * 
 * @see wasp.data.Variable
 * @author ywwong
 *
 */
public class Terminal extends Symbol {
    ...
    public static final int NUM_SPECIAL_TERMS = 6;
	...
	public boolean isWildcard() {
		return id == WILDCARD_NUM
		|| id == WILDCARD_UNUM
		|| id == WILDCARD_IDENT;
	}
	
	public boolean isWildcardMatch() {
		return isNum() || isUnum() || isIdent();
	}
	...
	/**
	 * Creates and returns a terminal symbol that indicates a sentence boundary.
	 * 
	 * @return a terminal symbol that indicates a sentence boundary.
	 */
	...
	/**
	 * Creates and returns a terminal symbol that stands for <i>lambda</i> in a lambda expression.
	 * 
	 * @return a terminal symbol for <i>lambda</i> in a lambda expression.
	 */
	public static Terminal lambda() {
		return new Terminal(LAMBDA, LAMBDA, (short) 0);
	}
	
	/**
	 * Returns the most general wildcard that matches this symbol.  If there are no wildcard
	 * that matches this symbol, then the symbol itself is returned.
	 * 
	 * @return the most general wildcard that matches this symbol.
	 */
	public Terminal toWildcard() {
		if (isNum())
			return wildcardNum();
		else if (isIdent())
