...
/**
 * An E-to-E translation model based on Pharaoh.  Training this model requires the Pharaoh training 
...
public class PhraseTranslationModel extends TranslationModel {
	...
	private class Settings {
	    ...
	    public File iniFile;
	    public String src;  // e.source
	    public String tar;  // e.target
	...
	private void writeInput(Examples examples) throws IOException {
        ...
        for (Iterator it = examples.iterator(); it.hasNext();) {
            ...
            // source is E.source
            Terminal[] E = ex.E(Config.getSourceNL());
            for (short j = 0; j < E.length; ++j)
            	if (!E[j].isBoundary()) {
            		srcSentOut.print(Dictionary.term(E[j].getId()));
            		srcSentOut.print(' ');
	            }
            srcSentOut.println();
            // target is E.target
            E = ex.E(Config.getTargetNL());
            for (short j = 0; j < E.length; ++j)
            	if (!E[j].isBoundary()) {
		...
		SCFG gramSrc = new SCFG();
		gramSrc.readInit();
        Rule[] rulesSrc = gramSrc.getRules();
        NL.useNL = Config.getTargetNL();
        SCFG gramTar = new SCFG();
        gramTar.readInit();
        Rule[] rulesTar = gramTar.getRules();
        for (int i = 0; i < rulesSrc.length; ++i)
        	if (rulesSrc[i].countArgs() == 0 && !rulesSrc[i].isWildcard()) {
        		Production prodSrc = rulesSrc[i].getProduction();
        		for (int j = 0; j < rulesTar.length; ++j)
        			if (prodSrc.equals(rulesTar[j].getProduction())) {
        	        	// source is E.source
        				Symbol[] E = rulesSrc[i].getE();
        				for (short k = 0; k < E.length; ++k)
        					if (!((Terminal) E[k]).isBoundary()) {
        						srcSentOut.print(Dictionary.term(E[k].getId()));
        						srcSentOut.print(' ');
        					}
        				srcSentOut.println();
			        	// target is E.target
			        	...
			        	for (short k = 0; k < E.length; ++k)
			        		if (!((Terminal) E[k]).isBoundary()) {
			            		tarSentOut.print(Dictionary.term(E[k].getId()));
			            		tarSentOut.print(' ');
			        		}
			        	tarSentOut.println();
		        	}
        ...
        for (Iterator it = examples.iterator(); it.hasNext();) {
            Example ex = (Example) it.next();
            Terminal[] Esrc = ex.E(Config.getSourceNL());
            Terminal[] Etar = ex.E(Config.getTargetNL());
            Production[] F = ex.F.lprods;
            for (short j = 0; j < F.length; ++j)
            	if (F[j].isWildcardMatch())
            		DOUBLE_LOOP2:
            		for (short k = 0; k < Esrc.length; ++k)
            			for (short l = 0; l < Etar.length; ++l)
            				if (Esrc[k].equals(Etar[l]) && Esrc[k].equals(F[j].getRhs((short) 0))) {
					            // source is E.source
					            srcSentOut.println(Dictionary.term(Esrc[k].getId()));
					            // target is E.target
					            tarSentOut.println(Dictionary.term(Etar[k].getId()));
					            break DOUBLE_LOOP2;
			            	}
        }
        srcSentOut.close();
