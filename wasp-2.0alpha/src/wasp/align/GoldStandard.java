...
/**
 * A word alignment model that simply uses the gold-standard word alignments if available.
 * 
 * @author ywwong
 *
 */
public class GoldStandard extends WordAlignModel {
	...
	public void train(Examples examples) throws IOException {
		...
		for (Iterator it = examples.iterator(); it.hasNext();) {
			...
			if (parse != null) {
				Meaning F = ex.F.replace((short) 0, parse);
				if (!F.linear[0].treeEquals(ex.F.linear[0])) {
					logger.fine("example "+ex.id+": MR is updated based on given MRL parse");
					ex.F = F;
				}
			}
	...
	private ArrayList readAugsyn(String str) {
		String[] line = new NL().tokenizeSyn(str);
		Int index = new Int(0);
	...
	private NTo1WordAlign toWordAlign(ArrayList augsyms, Example ex) {
		...
		for (short i = 0; i < augsyms.size(); ++i) {
			...
			if (augsym.isLinked()) {
				...
				if (augsym.nodeLhs != ex.F.lprods[augsym.nodeId].getLhs().getId()) {
